...
from app.keyboards.goals import goal_picker_kb
...
async def add_workout_type(message: Message, session: AsyncSession) -> None:
    ...
    await message.answer(f"Добавил новый тип: <b>{name}</b>", reply_markup=get_main_menu(message.from_user.id))
...
async def goal_info(message: Message, session: AsyncSession) -> None:
    ...
    await message.answer(
        f"Текущая цель: <b>{user.weekly_goal}</b> тренировок в неделю.\nВыбери новую цель кнопками ниже.",
        reply_markup=goal_picker_kb(user.weekly_goal),
    )


@router.callback_query(lambda c: c.data and c.data.startswith("goal:set:"))
async def set_goal_button(callback: CallbackQuery, session: AsyncSession) -> None:
...
async def set_goal_command(message: Message, session: AsyncSession) -> None:
    user = await _ensure_current_user(session, message)
    try:
        ...
        await message.answer("Используй /goal от 1 до 7. Или просто нажми кнопку «Цель».")
        return
    user.weekly_goal = goal
    await session.commit()
    await message.answer(f"Цель обновил: <b>{goal}</b> тренировок в неделю", reply_markup=get_main_menu(message.from_user.id))
