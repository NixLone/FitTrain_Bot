...
from app.services.reminder_suggestions import get_suggested_hours
...
from app.services.users import ensure_user, get_user_by_tg_id
...
def _quick_date_options() -> list[tuple[str, str]]:
    labels = []
    now = moscow_now()
    names = ["Сегодня", "Завтра"]
    for offset in range(5):
        day = now + timedelta(days=offset)
        label = names[offset] if offset < len(names) else day.strftime("%d.%m")
        labels.append((f"{label} · {day.strftime('%d.%m')}", f"quick:date:{day.strftime('%Y-%m-%d')}"))
    return labels


async def _ensure_current_user(session: AsyncSession, tg_user) -> object:
...
async def reminders_create(callback: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    await state.set_state(CreateReminderSG.choosing_quick_date)
    await callback.message.answer(
        "Выбери день из ближайших 5 дней или перейди в гибкую настройку:",
        reply_markup=quick_dates_kb(_quick_date_options()),
    )
...
@router.callback_query(lambda c: c.data == "quick:manual")
async def reminder_manual_mode(callback: CallbackQuery, state: FSMContext) -> None:
...
@router.callback_query(lambda c: c.data and c.data.startswith("quick:date:"))
async def quick_date_selected(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    user = await get_user_by_tg_id(session, callback.from_user.id)
    if not user:
        await callback.message.answer("Сначала нажми /start")
        await callback.answer()
        return
    date_value = callback.data.split(":", 2)[2]
    await state.update_data(schedule_type="one_time", date=date_value, quick_mode=True)
    await state.set_state(CreateReminderSG.choosing_workout_type)
    types_ = await list_available_workout_types(session, user.id)
    kb = simple_choice_kb([(item.name, f"rem_type:{item.id}:{item.name}") for item in types_], row_width=2)
...
async def choose_schedule(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    await state.update_data(schedule_type=schedule_type, quick_mode=False)
...
async def choose_reminder_workout_type(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    _, workout_type_id, workout_name = callback.data.split(":", 2)
    await state.update_data(workout_type_id=int(workout_type_id), workout_name=workout_name)
    ...
    if data.get("quick_mode"):
        ...
        if not user:
            ...
            await callback.answer()
            return
        hours = await get_suggested_hours(session, user.id)
        options = [(f"{hour:02d}:00", f"time:{hour:02d}:00") for hour in hours]
        await state.set_state(CreateReminderSG.choosing_time)
        await callback.message.answer(
            "Выбери время или отправь его вручную. Можно просто `18:00` или сразу `07.04.2026 18:00`.",
            reply_markup=suggested_times_kb(options),
        )
    else:
        await state.set_state(CreateReminderSG.entering_title)
        await callback.message.answer(f"Введи короткое название напоминания. Например: {workout_name}")
    await callback.answer()
...
async def reminder_title(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Теперь введи текст напоминания. Например: Пора на тренировку")
...
async def reminder_message(message: Message, state: FSMContext) -> None:
    ...
    if schedule_type == "weekly":
        ...
        await message.answer("Введи дни недели числами 1-7 через запятую. Например: 1,3,5")
    elif schedule_type == "one_time":
        await state.set_state(CreateReminderSG.entering_date)
        await message.answer("Введи дату в формате ДД.ММ.ГГГГ. Например: 20.04.2026")
    else:
...
async def reminder_one_time_date(message: Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer("Формат даты: 20.04.2026")
        return
...
async def reminder_time_pick(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    value = callback.data.split(":", 1)[1]
    if value == "manual":
        await callback.message.answer("Отправь время `18:00` или дату и время `07.04.2026 18:00`.")
        await callback.answer()
        return
    await _finish_quick_reminder(callback.message, session, state, value)
    await callback.answer()


@router.message(CreateReminderSG.choosing_time)
async def reminder_quick_time(message: Message, session: AsyncSession, state: FSMContext) -> None:
    await _finish_quick_reminder(message, session, state, message.text.strip())


async def _finish_quick_reminder(message: Message, session: AsyncSession, state: FSMContext, raw_value: str) -> None:
    data = await state.get_data()
    ...
    try:
        if " " in raw_value:
            local_dt = parse_datetime_ddmmyyyy_hhmm(raw_value)
        else:
            remind_time = parse_time_hhmm(raw_value)
            year, month, day = map(int, date_value.split("-"))
            local_dt = datetime(year=year, month=month, day=day, hour=remind_time.hour, minute=remind_time.minute, tzinfo=MOSCOW_TZ)
    except Exception:
        await message.answer("Формат времени: 18:00 или 07.04.2026 18:00")
        return
    ...
    reminder = await create_reminder(
        ...
        title=f"{workout_name}",
        message_text=f"Напоминание о тренировке: {workout_name}",
        schedule_type="one_time",
        specific_date_local=local_dt,
        remind_time=local_dt.timetz().replace(tzinfo=None),
    )
    await state.clear()
    await message.answer(
        f"Напоминание создано.\nСледующий запуск: <b>{format_moscow_dt(reminder.next_run_at)}</b>",
...
async def reminder_time(message: Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    if data["schedule_type"] == "weekly":
        ...
        kwargs["specific_date_local"] = datetime(
            year=year,
            month=month,
            day=day,
...
async def event_complete_comment(message: Message, state: FSMContext) -> None:
    ...
    await state.set_state(CompleteEventSG.entering_mood)
    await message.answer("Как прошло по ощущениям?", reply_markup=mood_kb())


@router.message(CompleteEventSG.entering_mood)
async def event_complete_mood(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Когда была тренировка? Напиши 'сейчас' или дату 17.03.2026 19:30", reply_markup=get_main_menu())
...
async def event_skip_reason(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    await callback.message.answer("Пропуск записал.", reply_markup=get_main_menu())
    await callback.answer()
...
async def event_reschedule_choice(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    await callback.message.answer(f"Перенес. Новое время: <b>{new_dt.strftime('%d.%m.%Y %H:%M')}</b>", reply_markup=get_main_menu())
    await callback.answer()
...
async def event_reschedule_custom(message: Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(f"Перенес на <b>{message.text}</b>", reply_markup=get_main_menu())
