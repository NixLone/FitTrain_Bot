...
async def admin_dashboard(message: Message, session: AsyncSession) -> None:
    ...
    lines = [
        "<b>Админка</b>",
        "",
        f"Пользователей: <b>{data['users_count']}</b>",
        f"Новых за 7 дней: <b>{data['new_users_week']}</b>",
        f"Активных напоминаний: <b>{data['active_reminders']}</b>",
        f"Записей тренировок за 7 дней: <b>{data['workouts_week']}</b>",
    ]
...
async def open_web_panel(message: Message) -> None:
    if not _is_admin(message.from_user.id):
        await message.answer(
            "Веб-панель доступна только администраторам.",
            reply_markup=get_main_menu(message.from_user.id),
        )
        return
    ...
    await message.answer(
        ...
        "Она действует 15 минут и перестает работать после использования.",
