...
async def _ensure_current_user(session: AsyncSession, tg_user):
...
async def reminders_list(callback: CallbackQuery, session: AsyncSession) -> None:
    user = await _ensure_current_user(session, callback.from_user)
...
async def choose_reminder_workout_type(
...
async def reminder_time_pick(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    await _finish_quick_reminder(callback.message, callback.from_user, session, state, value)
    await callback.answer()
...
async def reminder_quick_time(message: Message, session: AsyncSession, state: FSMContext) -> None:
    await _finish_quick_reminder(message, message.from_user, session, state, message.text.strip())


async def _finish_quick_reminder(
    message: Message,
    tg_user,
    session: AsyncSession,
    state: FSMContext,
    ...
    data = await state.get_data()
    user = await _ensure_current_user(session, tg_user)
    ...
    await message.answer(
        ...
        reply_markup=get_main_menu(tg_user.id),
    )
...
async def reminder_time(message: Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    user = await _ensure_current_user(session, message.from_user)
    kwargs = {
        "session": session,
        "user_id": user.id,
        "workout_type_id": data["workout_type_id"],
...
async def event_complete_mood(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "Когда была тренировка? Напиши 'сейчас' или дату 17.03.2026 19:30",
        reply_markup=get_main_menu(message.from_user.id),
    )
...
async def event_complete_finish(message: Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    if not event:
        await state.clear()
        await message.answer("Событие не найдено", reply_markup=get_main_menu(message.from_user.id))
        return
...
async def event_skip_reason(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    await callback.message.answer("Пропуск записал.", reply_markup=get_main_menu(callback.from_user.id))
    await callback.answer()
...
async def event_reschedule_choice(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        f"Перенес. Новое время: <b>{new_dt.strftime('%d.%m.%Y %H:%M')}</b>",
