...
def _render_progress(summary: dict) -> str:
    ...
    if summary["first_weight"] is not None:
        lines.append(f"Стартовый вес: <b>{summary['first_weight']}</b> кг")
    if summary["latest_weight"] is not None:
        lines.append(f"Текущий вес: <b>{summary['latest_weight']}</b> кг")
    ...
    lines.append("")
    lines.append(f"Всего за неделю: <b>{summary['completed_week']}</b> тренировок")
...
async def show_progress(message: Message, session: AsyncSession) -> None:
    ...
    await message.answer(_render_progress(summary), reply_markup=get_main_menu(message.from_user.id))
